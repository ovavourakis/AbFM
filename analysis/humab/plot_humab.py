...
# plot
fig, axes = plt.subplots(nrows=2, ncols=2, dpi=300, figsize=(15, 10))
fig.suptitle('Percent Labelled "Human" by Hu-mAb', fontsize=24)  # Adjusted y to leave more space
chain_labels = ['Full Fv', 'Heavy Chain', 'Kappa Light Chain', 'Lambda Light Chain']
...
for i, (title, frames) in enumerate([('Full Fv Stats:', [f.copy() for f in to_consider]),
    ...
    # num_per_bin = frame_trainorig['seq_len_bins'].value_counts()
    # print('\nN gensamples per bin (de novo):\n', num_per_bin)
    ...
    if i == 0 or i == 2:
        ax.set_ylabel('Percent', fontsize=14)  # Add y-axis label to the first subplot in each row
    ...
    # grey dotted line
    ax.axvline(x=overall_x - 0.5, color='grey', linestyle='dotted')

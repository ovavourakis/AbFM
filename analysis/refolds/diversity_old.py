# DEPRECATED: This script is no longer used, but is kept for reference.
...
#     print(f'{len(cdr_df)*len(cdr_df)} pairwise RMSDs to compute...')

#     return [len(cdr_df)*len(cdr_df)/2]
    
#     # print('Precomputing sequence indices and coordinates...')
#     # seq_indices, coords, vh_lens = {}, {}, {}
#     # for idx, row in tqdm(cdr_df.iterrows()):
#     #     seq_indices[idx] = numbering_to_region_index(row['domain_numbering'], row['seq'], row['chain'])
#     #     vh_lens[idx] = find_corresponding_vh_lens(anarci_df, row, row)
#     #     indices = [j + (vh_lens[idx][0]*4 if row['chain'] == 'L' else 0) for i in seq_indices[idx][cdr_type] for j in range(i*4, i*4+4)]
#     #     coords[idx] = get_backbone_coordinates(row['pdb_file'])[indices]
    
#     # # vectorized computation of pairwise RMSDs
#     # print('Computing pairwise RMSDs...')
#     # for i in tqdm(range(len(cdr_df))):
#     #     for j in range(i + 1, len(cdr_df)):
#     #         coords1 = coords[i]
#     #         coords2 = coords[j]
#     #         sup, new_coords2 = superpose_strucs(coords1, coords2)
#     #         rmsd = get_rmsd(coords1, new_coords2)
...
# #     print(f"Computing pairwise rmsds for {cdr_type}...")
# #     cdr_df = anarci_df[anarci_df[cdr_type] > 0] # filter out NaN-lengths from other chain type
# #     for cdr_len, cdr_df_group in tqdm(cdr_df.groupby(cdr_type)):

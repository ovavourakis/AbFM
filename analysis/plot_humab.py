...
#     bars2 = axes[i].bar([x + width for x in range(len(pc_human_traingen_bins))], pc_human_traingen_bins.values, color='black', alpha=0.5, label='Trainset AbMPNN', width=width)
#     bars3 = axes[i].bar([x + 2*width for x in range(len(pc_human_trainorig_bins))], pc_human_trainorig_bins.values, color='black', alpha=1, label='Trainset Original', width=width)

#     # overall bar
#     overall_x = len(labels)  # position for the 'Overall' bar
#     bars4 = axes[i].bar(overall_x, pc_human, color=color, alpha=1, width=width)
#     bars5 = axes[i].bar(overall_x + width, pc_human_traingen, color='black', alpha=0.5, width=width)
#     bars6 = axes[i].bar(overall_x + 2*width, pc_human_trainorig, color='black', alpha=1, width=width)
#     # grey dotted line
